...
import { driftObservationFromAuthenticatedVisualFinding, runDriftAudit, type DriftObservation } from "./audit";
import { initializeGenomeWithCodex } from "./genome-service";
import {
  approveGenome,
...
export async function authenticateGovernanceReportSession(
  ...
  const expected = await releaseSessionCheckpoint(project, projection);
  if (governanceCheckpointMatches(project.id, checkpoint, expected)) return "AUTHENTICATED";
  // Release is derived from current source and a signed audit generation.
  // A canonical retained predecessor has no current authority; preserve the
  // row as unavailable instead of poisoning all later report rows.
  if (checkpoint.entityId === expected.entityId && checkpoint.rootFingerprint === expected.rootFingerprint && checkpoint.revision <= expected.revision) return "UNAVAILABLE";
  throw new Error("Release report session is not related to authenticated current truth");
}
...
async function evaluateProjectReleaseGate(
  ...
  const proofs = await Promise.all((report.verificationSessionIds ?? []).map((id) => loadGovernanceCaptureSession(project.rootPath, project.id, id)));
  const verifiedAt = [...proofs.map(({ updatedAt }) => updatedAt), ...registry.records.flatMap(({ lastVerified }) => lastVerified === undefined ? [] : [lastVerified])]
    .sort()
    .at(-1);
  const allRegistered = proofs.length > 0 ? report.expectedScope.every((scope) => registry.records.some((record) => record.route === scope.screen && scope.states.every((state) => record.requiredStates.includes(state)))) : registry.records.length > 0 && registry.records.every(({ driftStatus }) => driftStatus !== "NOT_VERIFIED");
  const allFresh = proofs.length > 0 ? report.expectedScope.every((scope) => scope.states.every((state) => proofs.some((proof) => proof.render.route === scope.screen && proof.state === state && proof.render.sourceRevision.available && proof.render.sourceRevision.worktreeFingerprint === freshEvidence.sourceRevisionFingerprint))) : allRegistered && registry.records.every((record) =>
  ...
  const critical = report.findings.some(({ severity, status }) => severity === "CRITICAL" && status !== "RESOLVED" && status !== "INTENTIONAL");
  ...
  const runtimeCheck = (name: "navigation" | "accessibility" | "functionalVerification") => proofs.length === 0 ? "NOT_VERIFIED" as const : proofs.some((proof) => proof.browserChecks[name] === "FAIL") ? "FAIL" as const : proofs.every((proof) => proof.browserChecks[name] === "PASS") ? "PASS" as const : "NOT_VERIFIED" as const;
  return evaluateReleaseGate({
    ...
    navigation: runtimeCheck("navigation"),
    accessibility: runtimeCheck("accessibility"),
    ...
    functionalVerification: runtimeCheck("functionalVerification"),
    projectId: project.id,
    ...
    approvedDecisionIds: decisions.decisions
      ...
      .map(({ id }) => id),
    polishDebt: deriveDocumentedPolishDebt(report.findings, decisions.decisions),
    ...
    evidence: {
      ...
      functionalVerification: evidence,
...
export async function initializeProjectGovernance(
  project: Project,
  dependencies: { createAnalysisStagingDirectory(rootPath: string): Promise<string> },
): Promise<GovernanceProjection> {
  if (await governanceIsInitialized(project.rootPath)) {
  ...
  const analysisWorkingDirectory = await dependencies.createAnalysisStagingDirectory(
    project.rootPath,
  );
...
export async function recordProjectDriftCheckpoint(project: Project): Promise<void> {
  await persistGovernanceSession(project, await driftSessionCheckpoint(project));
}
