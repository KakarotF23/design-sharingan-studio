...
async function validateImage(
  ...
  try {
    ...
    const signature = Buffer.alloc(Math.min(12, current.size));
...
function analysisPrompt(input: AnalyzeRenderInput): string {
  return [
    ...
    "Prioritize visual hierarchy, UX integrity, accessibility, and product fit in that order before reference intent or cosmetic similarity.",
    "Do not chase pixel similarity at the expense of UX, navigation, product consistency, accessibility, or the approved design system.",
...
export async function analyzeRender(
  ...
  const referencePaths = await Promise.all(
    input.referenceImages.map(({ imagePath }) =>
      validateImage(projectRoot, imagePath, ".design-sharingan/references", MAX_REFERENCE_BYTES)
    )
  );
