...
export async function inspectBrowserChecks(input: { baseUrl: string; route: string; state: string }): Promise<BrowserVerificationChecks> {
  ...
  try {
    ...
    const linkTargets = [...new Set(snapshot.links)];
    if (linkTargets.length > 8) navigation = false;
    for (const target of linkTargets.slice(0, 8)) { const result = await page.goto(target, { waitUntil: "domcontentloaded", timeout: 10_000 }); navigation = navigation && result?.ok() === true && new URL(page.url()).origin === base.origin; }
    ...
    return { navigation: navigation ? "PASS" : "FAIL", accessibility: accessible ? "PASS" : "FAIL", functionalVerification: errors.length > 0 || !snapshot.main || !response?.ok() ? "FAIL" : snapshot.controls > 0 ? "NOT_VERIFIED" : "PASS", evidence: [`Observed ${route}#${input.state}: HTTP ${response?.status() ?? "unavailable"}; exact route and ${linkTargets.length} local navigation targets checked.`, `Structural accessibility smoke: language, title, one H1, main landmark, control/link names and image alternatives ${accessible ? "passed" : "failed"}. Visual accessibility is assessed separately.`, `Read-only content smoke: main content ${snapshot.main ? "present" : "missing"}; ${errors.length} runtime errors; ${snapshot.controls} interactive controls. Interactive workflows are not exercised or claimed.`, ...errors] };
